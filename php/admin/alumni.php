...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($univId === -1 && $univBaru) {
        $univKotaBaru     = clean($_POST['universitas_kota']     ?? '');
        $univProvinsiBaru = clean($_POST['universitas_provinsi'] ?? '');
        ...
        if ($existing) {
            ...
            // Cek apakah kolom provinsi ada (auto-migrate)
            try {
                $db->exec("ALTER TABLE universitas ADD COLUMN provinsi VARCHAR(100) DEFAULT NULL AFTER kota");
            } catch (Exception $e) { /* kolom sudah ada */ }
            $db->prepare('INSERT INTO universitas (kode, nama, kota, provinsi) VALUES (?, ?, ?, ?)')
               ->execute([$kode, $univBaru, $univKotaBaru, $univProvinsiBaru]);
...
<head>
  ...
  <style>
    ...
    .univ-lainnya-input:focus{
      border-color:var(--accent-dark);
      box-shadow:0 0 0 3px rgba(59,108,244,.12);
      background:#fff;
    }
    .univ-lainnya-hint{
      font-size:12px;
    ...
    /* Province / City select dalam form lainnya */
    .wilayah-grid{
      display:grid;
    ...
    @media(max-width:520px){ .wilayah-grid{ grid-template-columns:1fr; } }
    .wilayah-grid .f-group label{ font-size:12px; font-weight:700; color:var(--text-soft); margin-bottom:4px; }
    ...
    .template-row { display: flex; align-items: center; gap: 10px; margin-top: 12px; font-size: 12.5px; color: var(--muted); flex-wrap: wrap; }
    .btn-template { display: inline-flex; align-items: center; gap: 6px; padding: 6px 14px; border: 1px solid var(--border); border-radius: 20px; font-size: 12px; font-weight: 700; color: var(--text-soft); background: var(--surface); cursor: pointer; text-decoration: none; transition: all .15s; }
    .btn-template:hover { border-color: var(--accent); color: var(--accent); background: var(--accent-light); }

    .format-hint { background: #f0f9ff; border: 1px solid #bae6fd; border-radius: var(--r-sm); padding: 12px 16px; margin-top: 14px; font-size: 12.5px; line-height: 1.7; color: #075985; }
    .format-hint code { background: #e0f2fe; padding: 1px 5px; border-radius: 4px; font-family: monospace; font-size: 12px; font-weight: 700; }

    .preview-wrap { margin-top: 18px; display: none; }
    .preview-wrap.show { display: block; }
    .preview-meta { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; flex-wrap: wrap; gap: 8px; }
    .preview-meta span { font-size: 13px; font-weight: 700; color: var(--text-soft); }
    .preview-badge { font-size: 11px; font-weight: 700; padding: 3px 10px; border-radius: 999px; }
    .preview-badge.ok  { background: var(--green-bg); color: var(--green-text); }
    .preview-badge.err { background: var(--red-bg);   color: var(--red-text); }
    .preview-table-wrap { max-height: 260px; overflow: auto; border: 1px solid var(--border); border-radius: var(--r-sm); }
    .preview-table { width: 100%; border-collapse: collapse; font-size: 12.5px; white-space: nowrap; }
    .preview-table th { position: sticky; top: 0; background: #f7fafc; padding: 8px 12px; text-align: left; font-weight: 700; font-size: 11px; text-transform: uppercase; letter-spacing: .4px; color: var(--muted); border-bottom: 1px solid var(--border); }
    .preview-table td { padding: 8px 12px; border-bottom: 1px solid #f1f6fa; vertical-align: middle; }
    ...
    .import-action-bar { display: none; align-items: center; gap: 12px; margin-top: 14px; padding-top: 14px; border-top: 1px solid var(--border); flex-wrap: wrap; }
    .import-action-bar.show { display: flex; }
    .btn-import-now { padding: 10px 22px; background: var(--green); color: white; border: none; border-radius: var(--r-sm); font-family: var(--font); font-size: 13px; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 8px; transition: background .15s, transform .12s; box-shadow: 0 2px 10px rgba(34,197,94,.3); }
    ...
    .btn-import-reset { padding: 10px 16px; background: var(--bg); color: var(--text-soft); border: 1px solid var(--border); border-radius: var(--r-sm); font-family: var(--font); font-size: 13px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px; }
    .import-progress-text { font-size: 12.5px; color: var(--muted); font-weight: 600; }

    .import-result { display: none; margin-top: 14px; border-radius: var(--r-sm); padding: 14px 16px; }
    ...
    .import-result-title { font-size: 14px; font-weight: 800; margin-bottom: 8px; display: flex; align-items: center; gap: 8px; }
    .import-result ul { margin: 8px 0 0 18px; padding: 0; font-size: 12px; line-height: 1.8; color: var(--red-text); }
  </style>
</head>
  ...
  <main class="content">
    ...
    <div class="import-panel" id="importPanel">
      ...
      <div class="import-panel-body">
        <div class="import-steps">
          <div class="import-step active" id="istep1"><span class="step-num">1</span> Pilih File</div>
          <div class="import-step-sep"></div>
          <div class="import-step" id="istep2"><span class="step-num">2</span> Preview Data</div>
          <div class="import-step-sep"></div>
          <div class="import-step" id="istep3"><span class="step-num">3</span> Import</div>
        </div>
        ...
        <div class="template-row">
          ...
          <a class="btn-template" onclick="downloadTemplate('csv')"><i class="fa-solid fa-file-csv"></i> Template CSV</a>
          <a class="btn-template" onclick="downloadTemplate('xlsx')"><i class="fa-regular fa-file-excel"></i> Template XLSX</a>
        </div>
        <div class="format-hint">
          <strong>Format kolom:</strong><br>
          <code>nama</code> · <code>nisn</code> (opsional) · <code>universitas_nama</code> · <code>prodi</code> · <code>jalur</code> · <code>angkatan</code><br>
          Nilai <code>jalur</code>: <code>SNBP</code> / <code>SNBT</code> / <code>Mandiri</code> / <code>Kedinasan</code>
        </div>
        <div class="preview-wrap" id="previewWrap">
          ...
          <div class="preview-table-wrap">
            <table class="preview-table"><thead id="previewThead"></thead><tbody id="previewTbody"></tbody></table>
          </div>
        </div>
        <div class="import-action-bar" id="importActionBar">
          ...
          <button class="btn-import-reset" onclick="resetImport()"><i class="fa-solid fa-rotate-left"></i> Ganti File</button>
      ...
      <form method="POST" action="alumni.php" id="alumniForm" onsubmit="return validateAlumniForm()">
        ...
        <div class="form-grid" style="margin-bottom:16px;">
          <div class="f-group">
            ...
            <input type="text" name="nama" placeholder="Ketik nama alumni..." required autocomplete="off"
        ...
        <div class="form-grid" style="margin-bottom:16px;">
          <div class="f-group" style="grid-column:span 2;">
            ...
            <input type="hidden" name="universitas_kota"       id="univHiddenKota"     value="">
            <input type="hidden" name="universitas_provinsi"   id="univHiddenProvinsi" value="">

            <!-- Combobox -->
            <div class="combobox-wrap" id="univComboWrap">
              <div class="combobox-input-row <?= $editUnivId ? 'has-val' : '' ?>">
                <i class="fa-solid fa-building-columns combobox-icon"></i>
                <input type="text" id="univSearch" class="combobox-input"
              ...
              <ul class="combobox-list" id="univDropdown" style="display:none;"></ul>
            </div>

            <!-- Form Universitas Lainnya (muncul saat pilih "Lainnya") -->
            <div class="univ-lainnya-wrap" id="univLainnyaWrap" style="display:none;">

              <div style="display:flex;align-items:center;gap:8px;margin-top:12px;margin-bottom:10px;">
                <i class="fa-solid fa-plus-circle" style="color:var(--accent);font-size:14px;flex-shrink:0;"></i>
                <span style="font-size:13px;font-weight:800;color:var(--text-soft);">Tambah universitas baru ke database</span>
              </div>

              <!-- Nama Universitas -->
              <div class="f-group" style="margin-bottom:10px;">
                <label>Nama Universitas / Perguruan Tinggi *</label>
                <input type="text" id="univNamaBaru" class="univ-lainnya-input"
              ...
              <div class="wilayah-grid">
                <div class="f-group">
                  <label>
                    <i class="fa-solid fa-map" style="color:var(--accent);font-size:11px;"></i>
                    Provinsi *
                  </label>
                  <select id="univProvinsiSelect" class="wilayah-select" onchange="onProvinsiChange()">
                ...
                <div class="f-group">
                  ...
                  <select id="univKotaSelect" class="wilayah-select" onchange="onKotaChange()" disabled>
              ...
              <p class="univ-lainnya-hint">
                ...
                Koordinat peta akan dicari otomatis berdasarkan nama universitas &amp; kota yang dipilih.
              </p>
            </div>
    ...
    <form method="GET" action="alumni.php">
      <div class="filter-row">
        ...
        <a href="alumni.php" class="reset-btn"><i class="fa-solid fa-xmark"></i> Reset</a>
        <?php endif; ?>
      </div>
    </form>
...
<script>
// ════════════════════════════════════════════════════════════
//  DATA WILAYAH INDONESIA — Provinsi → Kota
// ════════════════════════════════════════════════════════════
const WILAYAH_ID = {
  ...
  'Sumatera Barat':            ['Padang','Bukittinggi','Payakumbuh','Padang Panjang','Solok','Sawahlunto','Pariaman'],
  'Riau':                      ['Pekanbaru','Dumai'],
  'Kepulauan Riau':            ['Tanjung Pinang','Batam'],
  'Jambi':                     ['Jambi','Sungai Penuh'],
  'Sumatera Selatan':          ['Palembang','Lubuklinggau','Prabumulih','Pagaralam'],
  'Kepulauan Bangka Belitung': ['Pangkal Pinang'],
  'Bengkulu':                  ['Bengkulu'],
  'Lampung':                   ['Bandar Lampung','Metro'],
  'DKI Jakarta':               ['Jakarta Pusat','Jakarta Utara','Jakarta Barat','Jakarta Selatan','Jakarta Timur'],
  'Jawa Barat':                ['Bandung','Bekasi','Bogor','Depok','Cirebon','Sukabumi','Tasikmalaya','Banjar','Cimahi'],
...
// Isi dropdown provinsi saat halaman load
(function initProvinsiDropdown() {
  const sel = document.getElementById('univProvinsiSelect');
  if (!sel) return;
  Object.keys(WILAYAH_ID).sort().forEach(prov => {
...
function onProvinsiChange() {
  const prov    = document.getElementById('univProvinsiSelect').value;
  const kotaSel = document.getElementById('univKotaSelect');
  document.getElementById('univHiddenProvinsi').value = prov;
  document.getElementById('univHiddenKota').value     = '';

  kotaSel.innerHTML = '<option value="">— Pilih Kota —</option>';
  if (!prov) { kotaSel.disabled = true; return; }

  (WILAYAH_ID[prov] || []).sort().forEach(kota => {
    const opt = document.createElement('option');
    opt.value = kota;
...
function onKotaChange() {
  document.getElementById('univHiddenKota').value = document.getElementById('univKotaSelect').value;
}
...
function handleFile(file) {
  ...
  if (!['csv','xlsx','xls'].includes(ext)) { alert('Format tidak didukung. Gunakan .csv atau .xlsx'); return; }
  if (file.size > 5 * 1024 * 1024) { alert('File terlalu besar. Maks. 5 MB.'); return; }
  const reader = new FileReader();
  if (ext === 'csv') {
    reader.onload = e => buildPreview(parseCSV(e.target.result), file.name);
    reader.readAsText(file, 'UTF-8');
  } else {
    reader.onload = e => {
      const wb = XLSX.read(new Uint8Array(e.target.result), { type:'array' });
      buildPreview(XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { defval:'' }), file.name);
    };
...
function buildPreview(rows, fileName) {
  if (!rows || rows.length === 0) { alert('File kosong atau format tidak terbaca.'); return; }
  const normalized = rows.map(r => {
    const n = {};
    Object.keys(r).forEach(k => { n[k.toLowerCase().trim()] = String(r[k]).trim(); });
    return n;
  });
  parsedRows = normalized; validRows = [];
  let thead = '<tr>';
  ['#','Nama','NISN','Universitas','Prodi','Jalur','Angkatan','Status'].forEach(h => thead += `<th>${h}</th>`);
  thead += '</tr>';
  document.getElementById('previewThead').innerHTML = thead;
  let okCount = 0, errCount = 0, tbody = '';
  normalized.forEach((row, idx) => {
    const errs = validateRow(row);
    const isOk = errs.length === 0;
    if (isOk) { validRows.push(row); okCount++; } else errCount++;
    tbody += `<tr class="${isOk ? '' : 'row-error'}">
      ...
      <td class="${isOk?'row-status-ok':'row-status-err'}">${isOk ? '✓ OK' : '✗ ' + errs[0]}</td>
    </tr>`;
  });
  document.getElementById('previewTbody').innerHTML = tbody;
  document.getElementById('previewMeta').textContent = `📄 ${fileName} · ${normalized.length} baris`;
  ...
  document.getElementById('importBtnText').textContent = `Import ${okCount} Alumni`;
  setStep(2);
}
...
function validateRow(row) {
  ...
  if (!row.universitas_nama) errs.push('universitas kosong');
...
async function doImport() {
  ...
  try {
    const res  = await fetch('bulk_import.php', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ rows: validRows }) });
    ...
    if (data.inserted > 0) {
      ...
      titleEl.innerHTML  = `<i class="fa-solid fa-circle-check" style="color:var(--green);"></i> ${data.inserted} alumni berhasil diimpor!${data.skipped ? ` <span style="font-size:12px;color:var(--muted);"> · ${data.skipped} dilewati</span>` : ''}`;
    } else {
      resultEl.className = 'import-result show error-result';
      titleEl.innerHTML  = `<i class="fa-solid fa-circle-xmark" style="color:var(--red);"></i> Tidak ada data yang berhasil diimpor.`;
    }
    if (data.errors && data.errors.length) {
      bodyEl.innerHTML = '<ul>' + data.errors.map(e => `<li>${esc(e)}</li>`).join('') + '</ul>';
    } else { bodyEl.innerHTML = ''; }
    prog.textContent = ''; icon.className = 'fa-solid fa-check'; txt.textContent = 'Selesai!'; importDone = true; setStep(3);
    if (data.inserted > 0) setTimeout(() => { window.location.href = 'alumni.php'; }, 2200);
  } catch (err) {
    prog.textContent = 'Gagal terhubung ke server.'; btn.disabled = false;
    icon.className = 'fa-solid fa-cloud-arrow-up'; txt.textContent = 'Coba Lagi';
  }
}
...
function setStep(n) {
  [1,2,3].forEach(i => {
    ...
    el.className = 'import-step' + (i < n ? ' done' : i === n ? ' active' : '');
    el.querySelector('.step-num').innerHTML = i < n ? '<i class="fa-solid fa-check" style="font-size:9px;"></i>' : i;
  });
}
...
function downloadTemplate(type) {
  ...
  if (type === 'csv') {
    const blob = new Blob(['\uFEFF' + [header, ...sample].map(r => r.join(',')).join('\n')], { type:'text/csv;charset=utf-8;' });
...
(function() {
  ...
  function pickItem(li) {
    ...
    if (lain) {
      ...
      // Reset wilayah fields setiap kali buka
      const pSel = document.getElementById('univProvinsiSelect');
      const kSel = document.getElementById('univKotaSelect');
      if (pSel) pSel.value = '';
      if (kSel) { kSel.innerHTML = '<option value="">— Pilih Kota —</option>'; kSel.disabled = true; }
      document.getElementById('univHiddenProvinsi').value = '';
      document.getElementById('univHiddenKota').value     = '';
  ...
  searchInput.addEventListener('input', function() {
    isLainnya = false; hiddenId.value = '';
  ...
  window.clearUniv = function() {
    searchInput.value = ''; hiddenId.value = ''; hiddenNama.value = '';
    lainnyaInput.value = '';
  ...
  window.validateAlumniForm = function() {
    ...
    if (hiddenId.value === '-1') {
      ...
      const provinsi = document.getElementById('univHiddenProvinsi').value;
      const kota     = document.getElementById('univHiddenKota').value;
      if (!provinsi) {
        alert('Pilih provinsi untuk universitas baru.');
        document.getElementById('univProvinsiSelect').focus();
        return false;
      }
      if (!kota) {
        alert('Pilih kota untuk universitas baru.');
        document.getElementById('univKotaSelect').focus();
        return false;
      }
    }
    return true;
...
// ════════════════════════════════════════════════════════════
//  CUSTOM DROPDOWN FOR FILTER SELECTS
// ════════════════════════════════════════════════════════════
document.addEventListener('DOMContentLoaded', function() {
  document.querySelectorAll('select.modern-select').forEach(select => {
    ...
    if (select.classList.contains('select-filter')) {
      wrapper.style.width = 'auto'; wrapper.style.minWidth = '160px';
    }
    ...
    if (select.classList.contains('select-filter')) {
      selectedDiv.style.cssText = 'display:flex;justify-content:space-between;align-items:center;background-image:none;padding-right:18px;';
    }
    const selectedText = document.createElement('span');
    selectedText.textContent = (select.options[select.selectedIndex] || select.options[0]).text;
    const icon = document.createElement('i');
    icon.className = 'fa-solid fa-chevron-down';
    if (select.classList.contains('select-filter')) { icon.style.marginLeft = '12px'; icon.style.color = 'var(--accent)'; }
    ...
    selectedDiv.addEventListener('click', e => {
      ...
      document.querySelectorAll('.custom-dropdown.open').forEach(el => { if (el !== wrapper) el.classList.remove('open'); });
